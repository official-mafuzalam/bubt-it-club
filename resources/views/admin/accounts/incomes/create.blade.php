<x-admin-layout>
    <x-slot name="main">
        @php
            $isEdit = isset($income);
            $pageTitle = $isEdit ? 'Edit Income' : 'Create New Income';
        @endphp
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                    {{ $isEdit ? 'Update the income details' : 'Add a new income' }}
                </p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden dark:bg-gray-800">
            <form action="{{ $isEdit ? route('admin.incomes.update', $income->id) : route('admin.incomes.store') }}"
                method="POST">
                @csrf
                @if ($isEdit)
                    @method('PUT')
                            ...
                            <select name="category_id" id="category_id" required
                                ...
                                @foreach ($categories as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('category_id', $income->category_id ?? '') == $item->id ? 'selected' : '' }}>
                                        {{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
