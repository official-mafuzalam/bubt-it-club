<x-admin-layout>
    <x-slot name="main">
        @section('page-title')
            <title>Income | Admin Panel</title>
        @endsection
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                    Manage all Income in the system
                </p>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        @forelse ($incomes as $income)
                            <tr class="bg-gray-100 dark:bg-gray-900">
                                <td class="px-6 py-4 whitespace-nowrap">{{ $income->date }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                    {{ $income->category->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">No incomes found
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="mt-4 bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
            <p class="text-lg font-semibold text-gray-800 dark:text-white">
                Total Income: <span class="text-green-600">{{ number_format($totalIncome, 2) }} ৳</span>
            </p>
        </div>
    </x-slot>
</x-admin-layout>
