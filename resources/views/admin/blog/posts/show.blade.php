            ...
            <div class="p-6">
                <div class="grid md:grid-cols-3 gap-8">
                    ...
                    <div class="md:col-span-2">
                        <!-- Post Header -->
                        <div class="mb-8">
                            <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">{{ $blog->title }}</h2>
                            ...
                            <div class="flex items-center space-x-4 mb-4">
                                <span
                                    ...
                                    {{ $blog->is_published ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200' }}">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6 sticky top-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-gray-900 dark:text-white">
                                        {{ $blog->created_at->format('M j, Y \a\t g:i A') }}
                                    </p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-gray-900 dark:text-white">
                                        {{ $blog->updated_at->format('M j, Y \a\t g:i A') }}
                                    </p>
                                </div>
                                ...
                                <div class="pt-4 border-t border-gray-200 dark:border-gray-600">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <form action="{{ route('admin.blog.posts.togglePublish', $blog->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit"
                                                class="w-full px-3 py-2 {{ $blog->is_published ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-green-600 hover:bg-green-700' }} text-white text-sm rounded-md">
                                                {{ $blog->is_published ? 'Unpublish' : 'Publish' }}
                                            </button>
                                        </form>
                                        @if ($blog->trashed())
                                            <form action="{{ route('admin.blog.posts.restore', $blog->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit"
                                            ...
                                            <form action="{{ route('admin.blog.posts.forceDelete', $blog->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                        ...
                                        @else
                                            <form action="{{ route('admin.blog.posts.destroy', $blog->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
